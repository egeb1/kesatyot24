% Eero Pietiläinen VTT transmitarray code to mesure when one element is
% active others inactive 21.6.2023
...
fid = fopen('Transmitarray_elemen_files_row54_phaseshift_chess_90deg_valein.txt','w');
A = (zeros(8,8)) - 50;
A(54) = -6;
TH = (zeros(8,8));
for a = 1:1
...
for m = 1:length(PROGPHASE(1,:))
    for n = 1:length(PROGPHASE(:,1))
        PROGPHASE(n,m) = dPHaz*(m-1) + dPHel*(n-1)+mod(m+n,2)*180; 
    end
end
%PROGPHASE(:) = PROGPHASE(:) + mod((1:64)',2)*180;
